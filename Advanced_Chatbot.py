...
# Initialize session state for controlling disclaimer and processing flag
...
if st.session_state.show_chat:
    ...
    disabled_dropdown = st.session_state.processing  # disable if processing is True
    ...
    # Only allow clicking Ask if not processing
    if st.session_state.processing:
        st.warning("Please wait for the current response to finish.")
    process_query_button = st.button("Ask this question", key="query_button", disabled=st.session_state.processing)
    
    # Initialize spaCy model for NER and load DistilGPT2 model and tokenizer
    ...
    # Process selected query from dropdown if Ask button is clicked and not processing
    if process_query_button and selected_query != "Choose your question":
        st.session_state.processing = True  # Lock further processing
    ...
    # Process free-text input from the bottom
